...
| A capped Pokémon that battles, and the experience it banked (2026-09-22) | Two more things the cap needed, both measured on the real X experience archive `a/2/1/7` (8 files of 404 bytes: the most any growth rate asks for level 100 is **1 640 000** and the biggest step between two levels is **68 116**, while `CappedStep` is **16 777 216** — about ten whole tables of room). (1) **A coded level was read through the experience earned since it**: the cap is a table and not a rule, so the game goes on adding what each battle gives to the coded value, and requiring an exact multiple of the step made one battle turn a capped Pokémon into level 100 — the answer for a value nothing else explains. `LevelOfCapped` now allows up to `MostOrdinaryExperience` of noise, the prototype window (`0x40000000 + 1..99`) still excluded. (2) **Experience banked at the cap is dropped when the cap lifts** (the item that was pending): a Pokémon held at the cap keeps earning, so it can be carrying several levels that would all be cashed in at once the moment the next step unlocks — which is what the cap was for. `ExperienceLevels.Trim` leaves it at the cap it was played under, `GameSettings.InstalledLevelCap` records the cap of the ROM that was built (a fact of the build, like `InstalledSeed`: an undo does not move it, and it is set after the save is adapted, so the adaptation sees the *old* cap), and `SaveUpdater` trims before fitting, reporting `ChangeKind.ExperienceTrimmed`. A level the cap coded is not touched: a code is not an amount, and a Pokémon caught above the cap gets its coded value from the game's own table. Verified on a copy of a real save: one carrying the experience of level 25 under a cap of 18 comes out at 18, and the same save played with no cap loses nothing. **The history restores the game options too**: `RestoreInto` only copied the randomization and the edits, so restoring a version rebuilt with the *current* level cap, shops and shiny; it brings the version's own back (keeping `InstalledLevelCap`, which is what is on disk), and `Fingerprint` counts them, since they are in the ROM as much as the edits are. Tests: `ExperienceLevelsTests` (13) and two in `ProjectHistoryTests`. |
| The cap is absolute, and it closes itself (2026-09-22, user: "if I overshoot with candies, the moment I close Citra, overwrite the levels back to what they should be") | **A Rare Candy ignores the cap** — it writes the next level's value straight in, which is why three candies took them from 17 to 20 under a cap of 18 — so the cap could be walked past by accident. `ExperienceLevels.Trim` now brings back **anything** above the cap, not only the experience banked in battle, and the floor is the cap **or the level the Pokémon was obtained at** (`MetLevel`), whichever is higher: the cap holds back training, it does not shrink what the game handed the player, so one caught or given above it keeps its level (and a candy on *that* is still undone, 31 → 30). A level above the floor is written in the coded form the game's own table holds up there, and `Stat_Level` follows, so the party shows the level it now has and its stats are recalculated from it. **It runs on its own when the emulator is closed**: `LevelCapViewModel` polls for the emulator every 3 s off the UI thread (`EmulatorUserFolders.RunningEmulators`) and on the edge running → gone runs the pass on the save, with the same safety as every other write (a `BeforeSaveEdit` history version, a backup, verification), writing nothing when nothing is over. It holds back, saying so, when the **save editor has unwritten changes** (they are not on disk and would be lost) or the **project has changes the ROM does not have yet** (the adaptation writes abilities and party stats from the session's data, which is then not what is installed — found while testing, before it could reach a save). Switch *"Al cerrar el emulador, devolver al tope los niveles que se hayan pasado"* in the level cap card (`GameSettings.TrimOverCap`, null = on, so older projects follow the cap); it changes no byte of the ROM, so it asks for no build. Verified on a copy of a real save: a party Pokémon candied to 21 under a cap of 18 comes back to 18 with its stats recalculated (HP 69 → 60), one caught at 30 is untouched, a boxed one that banked experience for 25 goes to 18, checksums valid, one backup, and a second pass writes nothing. A save the game has not written yet, a truncated one and one with broken checksums are all caught and reported instead of throwing. Tests: `ExperienceLevelsTests` (15). |
...
| Applying a difficulty froze the application (2026-09-22, user: "applying a difficulty crashes the app") | **It was not a crash but a hang**, and the event log says so: `AppHangB1`, "Pokemanager.App.exe stopped interacting with Windows" — which Windows declares after five seconds without an answer. **Measured, not guessed**: every `EditorSession.Set` raises `Changed`, the editor's handler calls `MarkDirty`, and that called `RefreshRomNotice` → `ProjectDiffersFromBuiltRom`, which **reads every version of the history from disk and hashes every edit of the project**: **3.95 ms per edit** on a project with 8 versions, so the 2287 values a Challenge profile writes were **nine seconds** of frozen interface, and far more on a real project with forty versions and thousands of edits. The handler also called `RefreshEditCounts`, ten notifications that each recount every edit. Both are things a batch **already does once when it ends** (`Batch.Dispose` called `RefreshRomNotice`), so `MarkDirty` now returns right after setting `IsDirty`/`romDirty` while `inBatch`, the counts are refreshed by the batch too, and the per-edit work is gone: the same profile is **293 ms** (104 writing, 178 the one undo snapshot of 233 KB, 11 the notice). It was the same bug in everything else that writes in bulk — "Put all back" over 4792 edits, the table sweeps of the game options — which is why those felt slow. |
| Difficulty profiles (2026-09-22, user: "difficulty levels with presets, and depending on the one chosen one set of settings or another is imported") | **Model A of `docs/game-ai.md`, built.** One choice when the run is set up — **Relajada · Normal · Desafío · Pesadilla** — in a card of **Opciones → Entrenadores** (user, same day: not on the build page; it belongs with the other trainer options, above the level and money ones it works with), carried by `UprOptionGroupViewModel.Difficulty` exactly as that group carries the shops on Items. It writes over every trainer what the player would otherwise set group by group in Advanced: Trainers. `Model/Difficulty/DifficultyProfiles`: a `DifficultyRule` per kind of trainer (**a null leaves the ROM's own value**) holding the AI level, the IVs 0-31, how many Full Restores the bag carries and from what level the last team member carries its own Mega Stone, plus one trainer level percentage for the whole game (`GameSettings.TrainerLevelPercent`, not an edit per trainer, so applying twice cannot pile up). The trainers are told apart by `TrainerRoles` — UPR ZX's own tag lists, so it works on a ROM with every trainer name randomized. **A profile owns its fields** (`ai`, `heldItems`, `item1…4`, the IVs of the six slots and their held item) and **puts them back before writing**, so one profile over another is that profile and not the two together, and **Normal is what takes a profile off**: it writes nothing and leaves the project with no trainer edit at all. Everything else — team, moves, money, a hand-edited level — is left exactly as it is. It is an ordinary batch of edits like the sweeps of Advanced: Game, so it shows in the trainers tab, Ctrl+Z takes it off in one step and it needs a ROM build, not a new seed. The card spells out what the selected profile writes, per kind, instead of asking for the choice blind. **Measured on the real X dump**: 784 of the 785 records parse, and they are **674 ordinary, 94 important, 16 bosses**; Relajada changes 1729 values in 752 trainers, Desafío 2287 in 784 and Pesadilla 2161 in 784 (the Full Restore is item **23**, read from the game's own item names). Tests: `RealDumpDifficultyTests` (4) — what each kind gets, that a profile over another is not the two piled up and that Normal leaves no edit, and that the team and the money survive. **Not in the profiles yet** (the document's model A asks for them): filling a boss's own moves from the ROM's learnset, growing a team to six, and Generation 7 EVs and natures. |
