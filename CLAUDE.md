...
| UPR's trainer level options hidden | The two UPR ZX options `TrainersLevelModified`/`TrainersLevelModifier` are **the same thing as the level percentage of Advanced: Trainers**, which is better (per trainer class, and changeable without randomizing again), so they are no longer offered twice (`UprOptionCatalog.Hidden`). A preset that sets them keeps its value and still applies it: what a shared `.rnqs` does is not changed behind its author's back. |
