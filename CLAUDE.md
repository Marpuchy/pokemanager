...
| **The application in two halves** (2026-09-21, user: "redistribute it, we touch trainers in many places, items in others") | The shell is **two tabs**: **Juego (ROM)** — what is built into the game and needs a rebuild — and **Partida** — what is touched while playing. `MainTabs` keeps the save at index 1, so the undo routing (`SaveTabIndex`) and the "open this Pokémon" jump from the start screen did not change. Juego holds *Aleatorizar* (the randomizer and its options), *Pokémon*, *Movimientos*, *Entrenadores*, *Reglas y tiendas* and *Datos*; Partida holds *Pokémon*, *Mochila*, *Entrenador*, *Pokédex* and *Locke*. What moved: the **Locke** is a tab of the run; **opening every box** and **starting over** sit with the run's other unlocks (Partida → Entrenador → Desbloqueos), not in the ROM's rules; the application's **item and shop options** (Mega Stones in the pool, every item, free Rare Candies, stones on sale) are one card in *Reglas y tiendas* instead of living among UPR ZX's item options, which now point there. |
