...
/// randomización en cada instalación. Es regenerable: borrar la caché solo cuesta volver a ejecutar UPR.
/// </summary>
public sealed class RandomizationCache(string root)
{
    ...
    /// <summary>
    /// Devuelve la salida en caché o ejecuta UPR para generarla. El preset se escribe en un archivo
    /// temporal porque UPR solo lo lee de disco.
    /// </summary>
        ...
        IProgress<string>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        string done = Path.Combine(dir, ".completo");
        string output = Path.Combine(dir, "salida");

        if (File.Exists(done) && FindResult(output, seed) is { } cached)
        {
        ...
        var result = await runner.RunAsync(presetFile, romFile, seed, output, progress, cancellationToken);
        await File.WriteAllTextAsync(done, DateTime.UtcNow.ToString("O"), cancellationToken);
        return result;
    }
